...
card* field::get_field_card(uint32 playerid, uint32 location, uint32 sequence) {
	...
	case LOCATION_PZONE: {
		if(sequence == 0) {
			card* pcard = player[playerid].list_szone[!(pduel->game_field->core.duel_options & DUEL_NO_EMZONE) ? (core.duel_options & SPEED_DUEL) ? 1 : 0 : 6];
			return pcard && pcard->current.pzone ? pcard : 0;
		} else if(sequence == 1) {
			card* pcard = player[playerid].list_szone[!(pduel->game_field->core.duel_options & DUEL_NO_EMZONE) ? (core.duel_options & SPEED_DUEL) ? 3 : 4 : 7];
			return pcard && pcard->current.pzone ? pcard : 0;
		} else
			return 0;
		break;
...
int32 field::is_location_useable(uint32 playerid, uint32 location, uint32 sequence) {
	...
	if (location == LOCATION_MZONE) {
		...
		if(pduel->game_field->core.duel_options & DUEL_NO_PZONE)
			return FALSE;
		else if(!(pduel->game_field->core.duel_options & DUEL_NO_EMZONE)) {
...
int32 field::get_useable_count(card* pcard, uint8 playerid, uint8 location, uint8 uplayer, uint32 reason, uint32 zone, uint32* list) {
	if(!(pduel->game_field->core.duel_options & DUEL_NO_EMZONE) && location == LOCATION_MZONE && pcard->current.location == LOCATION_EXTRA)
...
int32 field::get_spsummonable_count(card* pcard, uint8 playerid, uint32 zone, uint32* list) {
	if(!(pduel->game_field->core.duel_options & DUEL_NO_EMZONE) && pcard->current.location == LOCATION_EXTRA)
...
int32 field::get_mzone_limit(uint8 playerid, uint8 uplayer, uint32 reason) {
	...
	if(!(pduel->game_field->core.duel_options & DUEL_NO_EMZONE)) {
		max = 7;
...
int32 field::filter_matching_card(int32 findex, uint8 self, uint32 location1, uint32 location2, group* pgroup, card* pexception, group* pexgroup, uint32 extraargs, card** pret, int32 fcount, int32 is_target) {
	...
	for(uint32 p = 0; p < 2; ++p) {
		...
		if(location & LOCATION_PZONE) {
			for(int32 i = 0; i < 2; ++i) {
				pcard = player[self].list_szone[!(pduel->game_field->core.duel_options & DUEL_NO_EMZONE) ? (core.duel_options & SPEED_DUEL) ? (i == 0) ? 1 : 3 : i * 4 : i + 6];
...
int32 field::check_tuner_material(card* pcard, card* tuner, int32 findex1, int32 findex2, int32 min, int32 max, card* smat, group* mg) {
	...
	if(ct <= 0) {
		uint32 linked_zone = !(pduel->game_field->core.duel_options & DUEL_NO_EMZONE) ? get_linked_zone(playerid) | (1u << 5) | (1u << 6) : 0x1f;
