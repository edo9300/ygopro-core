...
LUA_STATIC_FUNCTION(EnableGlobalFlag, [[maybe_unused]] uint32_t flag_to_enable) {
	// noop
	return 0;
}

LUA_STATIC_FUNCTION(GetLP, playerid_t p) {
	lua_pushinteger(L, pduel->game_field->player[p].lp);
	return 1;
}
LUA_STATIC_FUNCTION(SetLP, playerid_t p, int32_t lp) {
	if(lp < 0) lp = 0;
...
LUA_STATIC_FUNCTION(IsTurnPlayer, playerid_t playerid) {
...
LUA_STATIC_FUNCTION(GetTurnCount, playerid_t playerid) {
	lua_pushinteger(L, pduel->game_field->infos.turn_id_by_player[playerid]);
	return 1;
}
LUA_STATIC_FUNCTION(GetDrawCount, playerid_t playerid) {
	lua_pushinteger(L, pduel->game_field->get_draw_count(playerid));
	return 1;
}
LUA_STATIC_FUNCTION(RegisterEffect, effect* peffect, playerid_t playerid) {
	pduel->game_field->add_effect(peffect, playerid);
	return 0;
}
LUA_STATIC_FUNCTION(RegisterFlagEffect, playerid_t playerid, uint32_t code, uint32_t reset, uint32_t flag, uint32_t count, std::optional<uint32_t> lab) {
	code = (code & 0xfffffffu) | 0x10000000u;
	if(count == 0)
		count = 1;
	...
	peffect->label.push_back(lab.value_or(0));
	pduel->game_field->add_effect(peffect, playerid);
	interpreter::pushobject(L, peffect);
	return 1;
}
LUA_STATIC_FUNCTION(GetFlagEffect, playerid_t playerid, uint32_t code) {
	code = (code & 0xfffffffu) | 0x10000000u;
	effect_set eset;
...
LUA_STATIC_FUNCTION(ResetFlagEffect, playerid_t playerid, uint32_t code) {
	code = (code & 0xfffffffu) | 0x10000000u;
...
LUA_STATIC_FUNCTION(SetFlagEffectLabel, playerid_t playerid, uint32_t code, int32_t lab) {
	code = (code & 0xfffffffu) | 0x10000000u;
	effect_set eset;
	pduel->game_field->filter_player_effect(playerid, code, &eset);
	if(!eset.size())
...
LUA_STATIC_FUNCTION(GetFlagEffectLabel, playerid_t playerid, uint32_t code) {
	code = (code & 0xfffffffu) | 0x10000000u;
	effect_set eset;
	pduel->game_field->filter_player_effect(playerid, code, &eset);
	if(eset.empty()) {
...
LUA_STATIC_FUNCTION(Destroy, std::variant<card*, group*> card_or_group, uint32_t reason, std::optional<uint16_t> dest, std::optional<playerid_none_t> reasonplayer) {
...
LUA_STATIC_FUNCTION(Remove, std::variant<card*, group*> card_or_group, std::optional<uint8_t> pos,
					uint32_t reason, std::optional<playerid_none_t> playerid, std::optional<playerid_none_t> reasonplayer) {
	check_action_permission(L);
...
LUA_STATIC_FUNCTION(SendtoGrave, std::variant<card*, group*> card_or_group, uint32_t reason,
					std::optional<playerid_none_t> playerid, std::optional<playerid_none_t> reasonplayer) {
	check_action_permission(L);
	auto sendto = [&, field = pduel->game_field](auto& obj) {
...
LUA_STATIC_FUNCTION(Summon, playerid_t playerid, card* pcard, bool ignore_count, std::optional<effect*> peffect,
	...
	pduel->game_field->summon(playerid, pcard, peffect.value_or(nullptr), ignore_count, min_tribute.value_or(0), zone.value_or(0x1f));
...
LUA_STATIC_FUNCTION(SpecialSummonRule, playerid_t playerid, card* pcard, std::optional<uint32_t> sumtype) {
	check_action_permission(L);
	if(pduel->game_field->core.effect_damage_step)
		return 0;
	pduel->game_field->core.summon_cancelable = FALSE;
	pduel->game_field->special_summon_rule(playerid, pcard, sumtype.value_or(0));
	if(pduel->game_field->core.current_chain.size()) {
		pduel->game_field->core.reserved = std::move(pduel->game_field->core.subunits.back());
		pduel->game_field->core.subunits.pop_back();
		pduel->game_field->core.summoning_card = pcard;
...
LUA_STATIC_FUNCTION(MSet, playerid_t playerid, card* pcard, bool ignore_count, std::optional<effect*> peffect,
	...
	if(pduel->game_field->core.effect_damage_step)
		return 0;
	pduel->game_field->core.summon_cancelable = FALSE;
	pduel->game_field->mset(playerid, pcard, peffect.value_or(nullptr), ignore_count, min_tribute.value_or(0), zone.value_or(0x1f));
	if(pduel->game_field->core.current_chain.size()) {
		pduel->game_field->core.reserved = std::move(pduel->game_field->core.subunits.back());
		pduel->game_field->core.subunits.pop_back();
		pduel->game_field->core.summoning_card = pcard;
...
LUA_STATIC_FUNCTION(SSet, playerid_t playerid, std::variant<card*, group*> card_or_group, std::optional<playerid_t> toplayer, std::optional<bool> confirm) {
	check_action_permission(L);
	...
	auto actual_toplayer = toplayer.value_or(playerid);
	pduel->game_field->emplace_process<Processors::SpellSetGroup>(playerid, actual_toplayer, pgroup, confirm.value_or(true), pduel->game_field->core.reason_effect);
	return yieldk({
		lua_pushinteger(L, pduel->game_field->returns.at<int32_t>(0));
		return 1;
	});
}
LUA_STATIC_FUNCTION(CreateToken, playerid_t playerid, uint32_t code) {
	check_action_permission(L);
...
LUA_STATIC_FUNCTION(SpecialSummon, std::variant<card*, group*> card_or_group, uint32_t sumtype, playerid_t sumplayer, playerid_t playerid,
					bool nocheck, bool nolimit, uint8_t positions, std::optional<uint32_t> zone) {
	check_action_permission(L);
...
LUA_STATIC_FUNCTION(SpecialSummonStep, card* pcard, uint32_t sumtype, playerid_t sumplayer,
					playerid_t playerid, bool nocheck, bool nolimit, uint8_t positions, std::optional<uint32_t> zone) {
	check_action_permission(L);
	pduel->game_field->special_summon_step(pcard, sumtype, sumplayer, playerid, nocheck, nolimit, positions, zone.value_or(0xff));
	return yieldk({
...
LUA_STATIC_FUNCTION(SendtoHand, std::variant<card*, group*> card_or_group, std::optional<playerid_none_t> playerid,
					uint32_t reason, std::optional<playerid_none_t> reasonplayer) {
	check_action_permission(L);
	auto sendto = [&, field = pduel->game_field](auto& obj) {
		field->send_to(obj, field->core.reason_effect, reason, reasonplayer.value_or(field->core.reason_player),
					   playerid.value_or(PLAYER_NONE), LOCATION_HAND, 0, POS_FACEUP);
	};
	if(auto ppcard = std::get_if<card*>(&card_or_group); ppcard)
		sendto(*ppcard);
	else
...
LUA_STATIC_FUNCTION(SendtoDeck, std::variant<card*, group*> card_or_group, std::optional<playerid_none_t> playerid,
					int32_t sequence, uint32_t reason, std::optional<playerid_none_t> reasonplayer) {
	check_action_permission(L);
	auto sendto = [&, field = pduel->game_field](auto& obj) {
		uint16_t location = (sequence == -2) ? 0 : LOCATION_DECK;
		field->send_to(obj, field->core.reason_effect, reason, reasonplayer.value_or(field->core.reason_player),
					   playerid.value_or(PLAYER_NONE), location, static_cast<uint32_t>(sequence), POS_FACEUP);
	};
	if(auto ppcard = std::get_if<card*>(&card_or_group); ppcard)
		sendto(*ppcard);
	else
...
LUA_STATIC_FUNCTION(SendtoExtraP, std::variant<card*, group*> card_or_group, std::optional<playerid_none_t> playerid,
					uint32_t reason, std::optional<playerid_none_t> reasonplayer) {
	check_action_permission(L);
	auto sendto = [&, field = pduel->game_field](auto& obj) {
		field->send_to(obj, field->core.reason_effect, reason, reasonplayer.value_or(field->core.reason_player),
					   playerid.value_or(PLAYER_NONE), LOCATION_EXTRA, 0, POS_FACEUP);
	};
	if(auto ppcard = std::get_if<card*>(&card_or_group); ppcard)
		sendto(*ppcard);
	else
...
LUA_STATIC_FUNCTION(Sendto, std::variant<card*, group*> card_or_group, uint16_t location, uint32_t reason,
					std::optional<uint8_t> position, std::optional<playerid_none_t> playerid, std::optional<uint32_t> sequence,
					std::optional<playerid_none_t> reason_player) {
	check_action_permission(L);
	auto sendto = [&, field = pduel->game_field](auto& obj) {
		field->send_to(obj, field->core.reason_effect, reason, reason_player.value_or(field->core.reason_player),
					   playerid.value_or(PLAYER_NONE), location, sequence.value_or(0), position.value_or(POS_FACEUP), TRUE);
	};
	if(auto ppcard = std::get_if<card*>(&card_or_group); ppcard)
		sendto(*ppcard);
	else
...
LUA_STATIC_FUNCTION(IsCanAddCounter, playerid_t playerid) {
	lua_pushboolean(L, pduel->game_field->is_player_can_action(playerid, EFFECT_CANNOT_PLACE_COUNTER));
	return 1;
}
LUA_STATIC_FUNCTION(IsCanAddCounter, playerid_t playerid, uint16_t countertype, uint32_t count, card* pcard) {
	lua_pushboolean(L, pduel->game_field->is_player_can_place_counter(playerid, pcard, countertype, count));
	return 1;
}
LUA_STATIC_FUNCTION(RemoveCounter, playerid_t rplayer, uint8_t self, uint8_t oppo,
...
LUA_STATIC_FUNCTION(IsCanRemoveCounter, playerid_t rplayer, uint8_t self, uint8_t oppo,
					uint16_t counter_type, uint32_t count, uint32_t reason) {
	lua_pushboolean(L, pduel->game_field->is_player_can_remove_counter(rplayer, nullptr, self, oppo, counter_type, count, reason));
	return 1;
}
LUA_STATIC_FUNCTION(GetCounter, playerid_t playerid, uint8_t self, uint8_t oppo, uint16_t counter_type) {
...
LUA_STATIC_FUNCTION(Release, std::variant<card*, group*> card_or_group, uint32_t reason, std::optional<playerid_none_t> reason_player) {
	check_action_permission(L);
	auto release = [&, field = pduel->game_field](auto& obj) {
		field->release(obj, pduel->game_field->core.reason_effect, reason, reason_player.value_or(pduel->game_field->core.reason_player));
	};
	if(auto ppcard = std::get_if<card*>(&card_or_group); ppcard)
		release(*ppcard);
	else
		release((*std::get_if<group*>(&card_or_group))->container);
	return yieldk({
		lua_pushinteger(L, pduel->game_field->returns.at<int32_t>(0));
		return 1;
	});
}
LUA_STATIC_FUNCTION(MoveToField, card* pcard, playerid_t move_player, playerid_t playerid, uint16_t destination, uint8_t positions, bool enable, std::optional<uint32_t> zone) {
	check_action_permission(L);
	pcard->enable_field_effect(false);
	pduel->game_field->adjust_instant();
	pduel->game_field->move_to_field(pcard, move_player, playerid, destination, positions, enable, 0, zone.value_or(0xff));
	return yieldk({
		lua_pushboolean(L, pduel->game_field->returns.at<int32_t>(0));
		return 1;
	});
}
LUA_STATIC_FUNCTION(ReturnToField, card* pcard, std::optional<uint8_t> position, std::optional<uint32_t> zone) {
	check_action_permission(L);
	...
	pduel->game_field->move_to_field(pcard, pcard->previous.controler, pcard->previous.controler, pcard->previous.location,
									 position.value_or(pcard->previous.position), TRUE, 1, zone.value_or(0xff), FALSE, LOCATION_REASON_TOFIELD | LOCATION_REASON_RETURN);
	return yieldk({
		lua_pushboolean(L, pduel->game_field->returns.at<int32_t>(0));
		return 1;
	});
}
LUA_STATIC_FUNCTION(MoveSequence, card* pcard, uint8_t seq, std::optional<uint16_t> location) {
	check_action_permission(L);
	uint16_t cur_loc = pcard->current.location;
	bool cur_pzone = pcard->current.pzone;
	auto actual_location = location.value_or(cur_loc);

	auto& field = *pduel->game_field;

	bool pzone = false;
	bool fzone = false;
	if(actual_location == LOCATION_PZONE) {
		if(!field.is_flag(DUEL_PZONE))
			lua_error(L, "LOCATION_PZONE passed with pendulum zones disabled");
		pzone = true;
		actual_location = LOCATION_SZONE;
	} else if(actual_location == LOCATION_FZONE) {
		fzone = true;
		actual_location = LOCATION_SZONE;
	}
	...
	if(pzone) {
		...
		if(seq != 0)
			lua_error(L, "Invalid sequence");
		seq = 5;
	} else if(actual_location == LOCATION_MZONE && field.is_flag(DUEL_EMZONE)) {
		if(seq > 6)
			lua_error(L, "Invalid sequence");
	} else if ((actual_location == LOCATION_MZONE || actual_location == LOCATION_SZONE) && seq > 4)
		lua_error(L, "Invalid sequence");
...
LUA_STATIC_FUNCTION(SwapSequence, card* pcard1, card* pcard2) {
	check_action_permission(L);
...
LUA_STATIC_FUNCTION(Activate, effect* peffect) {
	check_action_permission(L);
...
LUA_STATIC_FUNCTION(ConfirmDecktop, playerid_t playerid, uint32_t count) {
...
LUA_STATIC_FUNCTION(ConfirmExtratop, playerid_t playerid, uint32_t count) {
	const auto& player = pduel->game_field->player[playerid];
...
LUA_STATIC_FUNCTION(ConfirmCards, playerid_t playerid, std::variant<card*, group*> card_or_group) {
	auto ppgroup = std::get_if<group*>(&card_or_group);
	if(ppgroup && (*ppgroup)->container.empty())
		return 0;
	...
	if (ppgroup) {
		...
		auto* pcard = *std::get_if<card*>(&card_or_group);
		message->write<uint32_t>(1);
		message->write<uint32_t>(pcard->data.code);
		message->write<uint8_t>(pcard->current.controler);
		message->write<uint8_t>(pcard->current.location);
...
LUA_STATIC_FUNCTION(SortDecktop, playerid_t sort_player, playerid_t target_player, RangedInteger<uint8_t, 0, 64> count) {
	check_action_permission(L);
	if(count < 1)
		return 0;
...
LUA_STATIC_FUNCTION(SortDeckbottom, playerid_t sort_player, playerid_t target_player, RangedInteger<uint8_t, 0, 64> count) {
	check_action_permission(L);
	if(count < 1)
		return 0;
...
LUA_STATIC_FUNCTION(CheckEvent, uint32_t event, std::optional<bool> get_info) {
	if(!get_info.value_or(false)) {
		lua_pushboolean(L, pduel->game_field->check_event(event));
		return 1;
	} else {
		tevent pe;
		if(pduel->game_field->check_event(event, &pe)) {
			lua_pushboolean(L, 1);
...
LUA_STATIC_FUNCTION(RaiseEvent, std::variant<card*, group*> card_or_group, uint32_t code,
					std::optional<effect*> peffect, uint32_t r, uint8_t rp, uint8_t ep, uint32_t ev) {
	check_action_permission(L);
	auto raise = [&, field = pduel->game_field](auto& obj) {
		field->raise_event(obj, code, peffect.value_or(nullptr), r, rp, ep, ev);
	};
	if(auto* ppcard = std::get_if<card*>(&card_or_group); ppcard)
		raise(*ppcard);
	else
		raise((*std::get_if<group*>(&card_or_group))->container);
	pduel->game_field->process_instant_event();
	return yield();
}
LUA_STATIC_FUNCTION(RaiseSingleEvent, card* pcard, uint32_t code, std::optional<effect*> peffect,
					uint32_t r, uint8_t rp, uint8_t ep, uint32_t ev) {
	check_action_permission(L);
	pduel->game_field->raise_single_event(pcard, nullptr, code, peffect.value_or(nullptr), r, rp, ep, ev);
	pduel->game_field->process_single_event();
	return yield();
}
LUA_STATIC_FUNCTION(CheckTiming, uint32_t timing) {
	lua_pushboolean(L, (pduel->game_field->core.hint_timing[0] & timing) || (pduel->game_field->core.hint_timing[1] & timing));
	return 1;
}
...
LUA_STATIC_FUNCTION(IsEnvironment, uint32_t code, std::optional<uint8_t> playerid, std::optional<uint16_t> loc) {
	auto actual_playerid = playerid.value_or(PLAYER_ALL);
	auto actual_loc = loc.value_or(LOCATION_FZONE + LOCATION_ONFIELD);
	if(actual_playerid != 0 && actual_playerid != 1 && actual_playerid != PLAYER_ALL)
		lua_error(L, "Invalid playerid passed, allowed values are 0, 1, 3");
	auto RetTrue = [L] {
		lua_pushboolean(L, TRUE);
		return 1;
	};
	...
	auto CheckFzone = [&](uint8_t player) {
		if(actual_playerid == player || actual_playerid == PLAYER_ALL) {
	...
	auto ShouldApplyChangeEnv = [&]() {
		if((actual_loc & (LOCATION_FZONE | LOCATION_SZONE)) == 0)
			return false;
		return !(IsEnabled(pduel->game_field->player[0].list_szone[5]) || IsEnabled(pduel->game_field->player[1].list_szone[5]));
	};

	if(actual_loc & LOCATION_FZONE && (CheckFzone(0) || CheckFzone(1)))
		return RetTrue();

	if(actual_loc & LOCATION_SZONE && (CheckSzone(0) || CheckSzone(1)))
		return RetTrue();

	if(actual_loc & LOCATION_MZONE && (CheckMzone(0) || CheckMzone(1)))
		return RetTrue();
	...
	if(ShouldApplyChangeEnv()) {
		...
		if(eset.size()) {
			const auto& peffect = eset.back();
			if((actual_playerid == PLAYER_ALL || actual_playerid == peffect->get_handler_player()) && code == (uint32_t)peffect->get_value())
				return RetTrue();
		}
	}
	lua_pushboolean(L, FALSE);
...
LUA_STATIC_FUNCTION(Win, playerid_none_t playerid, uint32_t reason) {
	if (playerid == 0) {
...
LUA_STATIC_FUNCTION(Draw, playerid_t playerid, uint32_t count, uint32_t reason) {
	check_action_permission(L);
...
LUA_STATIC_FUNCTION(Damage, playerid_t playerid, int64_t amount, uint32_t reason, std::optional<bool> is_step, std::optional<playerid_none_t> reason_player) {
	check_action_permission(L);
	uint32_t actual_amount = 0;
	if(amount > 0)
		actual_amount = static_cast<uint32_t>(amount);
	pduel->game_field->damage(pduel->game_field->core.reason_effect, reason,
							  reason_player.value_or(pduel->game_field->core.reason_player), nullptr, playerid, actual_amount, is_step.value_or(false));
	return yieldk({
		lua_pushinteger(L, pduel->game_field->returns.at<uint32_t>(0));
		return 1;
	});
}
LUA_STATIC_FUNCTION(Recover, playerid_t playerid, int64_t amount, uint32_t reason, std::optional<bool> is_step, std::optional<playerid_none_t> reason_player) {
	check_action_permission(L);
	uint32_t actual_amount = 0;
	if(amount > 0)
		actual_amount = static_cast<uint32_t>(amount);
	pduel->game_field->recover(pduel->game_field->core.reason_effect, reason,
							   reason_player.value_or(pduel->game_field->core.reason_player), playerid, actual_amount, is_step.value_or(false));
	return yieldk({
		lua_pushinteger(L, pduel->game_field->returns.at<uint32_t>(0));
		return 1;
	});
...
LUA_STATIC_FUNCTION(Equip, playerid_t playerid, card* equip_card, card* target, std::optional<bool> flip_faceup, std::optional<bool> step) {
	check_action_permission(L);
...
LUA_STATIC_FUNCTION(PayLPCost, playerid_t playerid, uint32_t cost) {
	check_action_permission(L);
	pduel->game_field->emplace_process<Processors::PayLPCost>(playerid, cost);
	return yield();
}
LUA_STATIC_FUNCTION(DiscardDeck, playerid_t playerid, uint16_t count, uint32_t reason) {
	check_action_permission(L);
...
LUA_STATIC_FUNCTION(DiscardHand, playerid_t playerid, std::optional<Function> findex, uint16_t min, uint16_t max, uint32_t reason, std::variant<Nil, card*, group*> exception/*, ...*/) {
	...
	if(auto* ppgroup = std::get_if<group*>(&exception); ppgroup) {
		pexgroup = *ppgroup;
...
LUA_STATIC_FUNCTION(DisableShuffleCheck, std::optional<bool> disable) {
	pduel->game_field->core.shuffle_check_disabled = disable.value_or(true);
	return 0;
}
LUA_STATIC_FUNCTION(DisableSelfDestroyCheck, std::optional<bool> disable) {
	pduel->game_field->core.selfdes_disabled = disable.value_or(true);
	return 0;
}
LUA_STATIC_FUNCTION(ShuffleDeck, playerid_t playerid) {
	pduel->game_field->shuffle(playerid, LOCATION_DECK);
	return yield();
}
LUA_STATIC_FUNCTION(ShuffleExtra, playerid_t playerid) {
	pduel->game_field->shuffle(playerid, LOCATION_EXTRA);
	return yield();
}
LUA_STATIC_FUNCTION(ShuffleHand, playerid_t playerid) {
	pduel->game_field->shuffle(playerid, LOCATION_HAND);
	return yield();
}
LUA_STATIC_FUNCTION(ShuffleSetCard, group* pgroup) {
...
LUA_STATIC_FUNCTION(ChangeAttacker, card* new_attacker, std::optional<bool> no_increment_attack_announce) {
	...
	if(!no_increment_attack_announce.value_or(false)) {
...
LUA_STATIC_FUNCTION(ChangeAttackTarget, card* attack_target, std::optional<bool> no_increment_attack_announce) {
	...
	if(((attack_target && std::find(cv.begin(), cv.end(), attack_target) != cv.end()) || no_increment_attack_announce.value_or(false)) ||
		(!attack_target && !attacker->is_affected_by_effect(EFFECT_CANNOT_DIRECT_ATTACK))) {
		pduel->game_field->core.attack_target = attack_target;
		...
		if(attack_target) {
			if(attack_target->current.controler != turnp) {
				pduel->game_field->raise_single_event(attack_target, nullptr, EVENT_BE_BATTLE_TARGET, nullptr, REASON_REPLACE, 0, turnp, 0);
				pduel->game_field->raise_event(attack_target, EVENT_BE_BATTLE_TARGET, nullptr, REASON_REPLACE, 0, turnp, 0);
			}else{
				pduel->game_field->raise_single_event(attack_target, nullptr, EVENT_BE_BATTLE_TARGET, nullptr, REASON_REPLACE, 0, 1 - turnp, 0);
				pduel->game_field->raise_event(attack_target, EVENT_BE_BATTLE_TARGET, nullptr, REASON_REPLACE, 0, 1 - turnp, 0);
			}
			pduel->game_field->process_single_event();
			pduel->game_field->process_instant_event();
			message->write(attack_target->get_info_location());
		} else {
...
LUA_STATIC_FUNCTION(AttackCostPaid, std::optional<uint8_t> paid) {
	pduel->game_field->core.attack_cost_paid = paid.value_or(1);
	return 0;
}
LUA_STATIC_FUNCTION(ForceAttack, card* attacker, card* attack_target) {
	check_action_permission(L);
...
LUA_STATIC_FUNCTION(CalculateDamage, card* attacker, std::optional<card*> attack_target, std::optional<bool> new_attack) {
	check_action_permission(L);
	if(attacker == attack_target)
		return 0;
	pduel->game_field->emplace_process<Processors::DamageStep>(attacker, attack_target.value_or(nullptr), new_attack.value_or(false));
	return yield();
}
LUA_STATIC_FUNCTION(GetBattleDamage, playerid_t playerid) {
	lua_pushinteger(L, pduel->game_field->core.battle_damage[playerid]);
	return 1;
}
LUA_STATIC_FUNCTION(ChangeBattleDamage, playerid_t playerid, uint32_t amount, std::optional<bool> require_nonzero_damage) {
	if(require_nonzero_damage.value_or(true) && pduel->game_field->core.battle_damage[playerid] == 0)
		return 0;
	pduel->game_field->core.battle_damage[playerid] = amount;
	return 0;
}
LUA_STATIC_FUNCTION(ChangeTargetCard, uint8_t chain_index, group* pgroup) {
	pduel->game_field->change_target(chain_index, pgroup);
	return 0;
}
LUA_STATIC_FUNCTION(ChangeTargetPlayer, uint8_t chain_index, playerid_t playerid) {
	pduel->game_field->change_target_player(chain_index, playerid);
	return 0;
}
LUA_STATIC_FUNCTION(ChangeTargetParam, uint8_t chain_index, int32_t param) {
	pduel->game_field->change_target_param(chain_index, param);
	return 0;
}
...
LUA_STATIC_FUNCTION(NegateActivation, uint8_t chain_index) {
	lua_pushboolean(L, pduel->game_field->negate_chain(chain_index));
	return 1;
}
LUA_STATIC_FUNCTION(NegateEffect, uint8_t chain_index) {
	lua_pushboolean(L, pduel->game_field->disable_chain(chain_index));
	return 1;
}
LUA_STATIC_FUNCTION(NegateRelatedChain, card* pcard, uint32_t reset_flag) {
...
LUA_STATIC_FUNCTION(NegateSummon, std::variant<card*, group*> card_or_group) {
	uint8_t sumplayer = PLAYER_NONE;
	auto set_statuses = [&](card* pcard) {
	...
	if(auto ppcard = std::get_if<card*>(&card_or_group); ppcard)
		pduel->game_field->raise_event(*ppcard, event_code, reason_effect, REASON_EFFECT, reason_player, sumplayer, 0);
	else
		pduel->game_field->raise_event((*std::get_if<group*>(&card_or_group))->container, event_code, reason_effect, REASON_EFFECT, reason_player, sumplayer, 0);
	pduel->game_field->process_instant_event();
	return 0;
}
LUA_STATIC_FUNCTION(IncreaseSummonedCount) {
	++pduel->game_field->core.summon_count[pduel->game_field->core.reason_player];
	return 0;
}
LUA_STATIC_FUNCTION(IncreaseSummonedCount, card* pcard) {
	if(auto* pextra = pcard->is_affected_by_effect(EFFECT_EXTRA_SUMMON_COUNT); pextra != nullptr)
...
LUA_STATIC_FUNCTION(CheckSummonedCount) {
	...
	lua_pushboolean(L, pduel->game_field->core.summon_count[playerid] < pduel->game_field->get_summon_count_limit(playerid));
	return 1;
}
LUA_STATIC_FUNCTION(CheckSummonedCount, card* pcard) {
	uint8_t playerid = pduel->game_field->core.reason_player;
	lua_pushboolean(L, pcard->is_affected_by_effect(EFFECT_EXTRA_SUMMON_COUNT)
					|| (pduel->game_field->core.summon_count[playerid] < pduel->game_field->get_summon_count_limit(playerid)));
	return 1;
}
LUA_STATIC_FUNCTION(GetLocationCount, playerid_t playerid, uint8_t location, std::optional<uint8_t> uplayer, std::optional<uint32_t> reason, std::optional<uint32_t> zone) {
	uint32_t list = 0;
	lua_pushinteger(L, pduel->game_field->get_useable_count(nullptr, playerid, location,
															uplayer.value_or(pduel->game_field->core.reason_player),
															reason.value_or(LOCATION_REASON_TOFIELD),
															zone.value_or(0xff), &list));
	lua_pushinteger(L, list != 0);
	return 2;
}
LUA_STATIC_FUNCTION(GetMZoneCount, playerid_t playerid, std::variant<Nil, card*, group*> card_or_group, std::optional<playerid_t> uplayer, std::optional<uint32_t> reason, std::optional<uint32_t> zone) {
	bool swapped = false;
	...
	if(auto [mcard, mgroup] = expand_to_card_or_group(card_or_group); mcard || mgroup) {
	...
	uint32_t list = 0;
	lua_pushinteger(L, pduel->game_field->get_useable_count(nullptr, playerid, LOCATION_MZONE,
															uplayer.value_or(pduel->game_field->core.reason_player),
															reason.value_or(LOCATION_REASON_TOFIELD),
															zone.value_or(0xff), &list));
	lua_pushinteger(L, list != 0);
...
LUA_STATIC_FUNCTION(GetLocationCountFromEx, playerid_t playerid, std::optional<playerid_t> uplayer, std::variant<Nil, card*, group*> card_or_group,
					std::variant<Nil, card*, uint32_t> card_or_type, std::optional<uint32_t> zone) {
	bool swapped = false;
	const uint32_t default_loc = 0x1111 * pduel->game_field->is_flag(DUEL_3_COLUMNS_FIELD);
	uint32_t used_location[2] = { default_loc, default_loc };
	card_vector list_mzone[2];
	...
	card* scard = nullptr;
	if(auto* ppcard = std::get_if<card*>(&card_or_type); ppcard) {
		scard = *ppcard;
	} else if(auto* ptype = std::get_if<uint32_t>(&card_or_type); ptype) {
		use_temp_card = true;
		auto type = *ptype;
	...
	uint32_t list = 0;
	lua_pushinteger(L, pduel->game_field->get_useable_count_fromex(scard, playerid, uplayer.value_or(pduel->game_field->core.reason_player), zone.value_or(0xff), &list));
	lua_pushinteger(L, list != 0);
	if(swapped) {
		pduel->game_field->player[0].used_location = used_location[0];
		pduel->game_field->player[1].used_location = used_location[1];
...
LUA_STATIC_FUNCTION(GetUsableMZoneCount, playerid_t playerid, std::optional<uint8_t> uplayer) {
	...
	int32_t ct1 = pduel->game_field->get_tofield_count(nullptr, playerid, LOCATION_MZONE, actual_uplayer, LOCATION_REASON_TOFIELD, zone, &flag1);
	int32_t ct2 = pduel->game_field->get_spsummonable_count_fromex(nullptr, playerid, actual_uplayer, zone, &flag2);
	int32_t ct3 = field::field_used_count[~(flag1 | flag2) & 0x1f];
	int32_t count = ct1 + ct2 - ct3;
	int32_t limit = pduel->game_field->get_mzone_limit(playerid, actual_uplayer, LOCATION_REASON_TOFIELD);
	if(count > limit)
		count = limit;
	lua_pushinteger(L, count);
	return 1;
}
LUA_STATIC_FUNCTION(GetLinkedGroup, uint8_t rplayer, uint16_t location1, uint16_t location2) {
...
LUA_STATIC_FUNCTION(GetLinkedGroupCount, uint8_t rplayer, uint16_t location1, uint16_t location2) {
	if(rplayer != 0 && rplayer != 1)
		return 0;
	if(location1 == 1)
		location1 = LOCATION_MZONE;
...
LUA_STATIC_FUNCTION(GetLinkedZone, uint8_t playerid) {
	if(playerid != 0 && playerid != 1)
		return 0;
	lua_pushinteger(L, pduel->game_field->get_linked_zone(playerid));
	return 1;
}
LUA_STATIC_FUNCTION(GetFreeLinkedZone, uint8_t playerid) {
	if(playerid != 0 && playerid != 1)
		return 0;
...
LUA_STATIC_FUNCTION(GetCurrentChain, std::optional<bool> real) {
...
LUA_STATIC_FUNCTION(IsPhase, uint16_t phase) {
...
LUA_STATIC_FUNCTION(SkipPhase, playerid_t playerid, uint16_t phase, uint32_t reset, uint16_t count, std::optional<uint32_t> value) {
	...
	peffect->o_range = 0;
	peffect->reset_count = count;
	peffect->value = value.value_or(0);
	pduel->game_field->add_effect(peffect, playerid);
	return 0;
}
...
LUA_STATIC_FUNCTION(GetBattleMonster, uint8_t playerid) {
	card* attacker = pduel->game_field->core.attacker;
...
LUA_STATIC_FUNCTION(ChainAttack, std::optional<card*> chain_attack_target) {
	auto* attacker = pduel->game_field->core.attacker;
	...
	pduel->game_field->core.chain_attack_target = chain_attack_target.value_or(nullptr);
	return 0;
}
...
LUA_STATIC_FUNCTION(SelectFusionMaterial, playerid_t playerid, card* summoning_card, group* material_group,
					std::variant<Nil, card*, group*> forced_materials, std::optional<uint32_t> fusion_summon_flags) {
	check_action_permission(L);
	...
	if(auto* ppcard = std::get_if<card*>(&forced_materials); ppcard)
		...
		forced_materials_group = *ppgroup;
	pduel->game_field->emplace_process<Processors::SelectFusion>(playerid, material_group, fusion_summon_flags.value_or(PLAYER_NONE), forced_materials_group, summoning_card);
	return yieldk({
...
LUA_STATIC_FUNCTION(SetFusionMaterial, group* material_group) {
	pduel->game_field->core.fusion_materials = material_group->container;
	return 0;
}
LUA_STATIC_FUNCTION(GetRitualMaterial, playerid_t playerid, std::optional<bool> check_level) {
	auto pgroup = pduel->new_group();
	pduel->game_field->get_ritual_material(playerid, pduel->game_field->core.reason_effect, &pgroup->container, check_level.value_or(true));
	interpreter::pushobject(L, pgroup);
	return 1;
}
LUA_STATIC_FUNCTION(ReleaseRitualMaterial, group* pgroup, std::optional<bool> release_deck) {
	check_action_permission(L);
	pduel->game_field->ritual_release(pgroup->container, release_deck.value_or(false));
	return yield();
}
LUA_STATIC_FUNCTION(GetFusionMaterial, playerid_t playerid) {
	auto pgroup = pduel->new_group();
...
LUA_STATIC_FUNCTION(SetSelectedCard, std::variant<card*, group*> forced_materials) {
	if(auto* ppcard = std::get_if<card*>(&forced_materials); ppcard)
		pduel->game_field->core.must_select_cards = { *ppcard };
	else {
		auto* pgroup = *std::get_if<group*>(&forced_materials);
...
LUA_STATIC_FUNCTION(SetTargetCard, std::variant<card*, group*> forced_materials) {
	check_action_permission(L);
	chain* ch = pduel->game_field->get_chain(0);
	if(!ch)
		return 0;
	...
	effect* peffect = ch->triggering_effect;
	auto [pcard, pgroup] = expand_to_card_or_group(forced_materials);
	if(peffect->type & EFFECT_TYPE_CONTINUOUS) {
		if(pcard)
...
LUA_STATIC_FUNCTION(SetTargetPlayer, playerid_t playerid) {
	check_action_permission(L);
	chain* ch = pduel->game_field->get_chain(0);
	if(ch)
...
LUA_STATIC_FUNCTION(SetTargetParam, uint32_t param) {
	check_action_permission(L);
	chain* ch = pduel->game_field->get_chain(0);
	if(ch)
...
LUA_STATIC_FUNCTION(SetOperationInfo, uint32_t chain_index, uint64_t category, std::variant<Nil, card*, group*> object, uint8_t count, playerid_either_t playerid, int32_t param) {
	check_action_permission(L);
	chain* ch = pduel->game_field->get_chain(chain_index);
	if(!ch)
		return 0;
	optarget opt{ nullptr, count, playerid, param };
	if(!std::holds_alternative<Nil>(object)) {
		if(auto* ppcard = std::get_if<card*>(&object); ppcard)
			opt.op_cards = pduel->new_group(*ppcard);
		else
			opt.op_cards = pduel->new_group(*std::get_if<group*>(&object));
		...
		if(category == 0x200 && playerid == PLAYER_ALL && opt.op_cards->container.size() != 2)
	...
	ch->opinfos[category] = std::move(opt);
	return 0;
}
LUA_STATIC_FUNCTION(GetOperationInfo, uint32_t chain_index, uint64_t category) {
	chain* ch = pduel->game_field->get_chain(chain_index);
	if(!ch)
		return 0;
	auto oit = ch->opinfos.find(category);
	if(oit != ch->opinfos.end()) {
		const optarget& opt = oit->second;
		lua_pushboolean(L, true);
...
LUA_STATIC_FUNCTION(SetPossibleOperationInfo, uint32_t chain_index, uint64_t category, std::variant<Nil, card*, group*> object, uint8_t count, playerid_either_t playerid, int32_t param) {
	check_action_permission(L);
	chain* ch = pduel->game_field->get_chain(chain_index);
	if(!ch)
		return 0;
	...
	ch->possibleopinfos[category] = std::move(opt);
	return 0;
}
LUA_STATIC_FUNCTION(GetPossibleOperationInfo, uint32_t chain_index, uint64_t category) {
	chain* ch = pduel->game_field->get_chain(chain_index);
	if(!ch)
		return 0;
	auto oit = ch->possibleopinfos.find(category);
...
LUA_STATIC_FUNCTION(GetOperationCount, uint32_t chain_index) {
	chain* ch = pduel->game_field->get_chain(chain_index);
	if(!ch)
		return 0;
...
LUA_STATIC_FUNCTION(ClearOperationInfo, uint32_t chain_index) {
	check_action_permission(L);
	chain* ch = pduel->game_field->get_chain(chain_index);
	if(!ch)
		return 0;
...
LUA_STATIC_FUNCTION(Overlay, card* target, std::variant<card*, group*> materials, std::optional<bool> dont_transfer_materials) {
	check_action_permission(L);
	...
	if(auto* ppcard = std::get_if<card*>(&materials); ppcard) {
		...
		pduel->game_field->xyz_overlay(target, pcard, dont_transfer_materials.value_or(false));
	} else {
		auto* pgroup = *std::get_if<group*>(&materials);
		if(pgroup->has_card(target))
			lua_error(L, "Attempt to overlay a card with itself.");
		pduel->game_field->xyz_overlay(target, pgroup->container, dont_transfer_materials.value_or(false));
	}
	return yield();
}
LUA_STATIC_FUNCTION(GetOverlayGroup, playerid_t reason_player, uint8_t self, uint8_t oppo, std::optional<group*> targets) {
	auto pgroup = pduel->new_group();
	pduel->game_field->get_overlay_group(reason_player, self, oppo, &pgroup->container, targets.value_or(nullptr));
	interpreter::pushobject(L, pgroup);
	return 1;
}
LUA_STATIC_FUNCTION(GetOverlayCount, playerid_t reason_player, uint8_t self, uint8_t oppo, std::optional<group*> targets) {
	lua_pushinteger(L, pduel->game_field->get_overlay_count(reason_player, self, oppo, targets.value_or(nullptr)));
	return 1;
}
LUA_STATIC_FUNCTION(CheckRemoveOverlayCard, playerid_t remove_player, uint8_t self, uint8_t oppo, uint16_t count, uint32_t reason, std::optional<group*> targets) {
	lua_pushboolean(L, pduel->game_field->is_player_can_remove_overlay_card(remove_player, targets.value_or(nullptr), self, oppo, count, reason));
	return 1;
}
LUA_STATIC_FUNCTION(RemoveOverlayCard, playerid_t remove_player, uint8_t self, uint8_t oppo, uint16_t min, uint16_t max, uint32_t reason, std::optional<group*> targets) {
	check_action_permission(L);
	pduel->game_field->remove_overlay_card(reason, targets.value_or(nullptr), remove_player, self, oppo, min, max);
	return yieldk({
		lua_pushinteger(L, pduel->game_field->returns.at<int32_t>(0));
		return 1;
	});
}
LUA_STATIC_FUNCTION(Hint, uint8_t hint_type, playerid_t playerid, uint64_t description) {
	if(hint_type == HINT_OPSELECTED)
		playerid = 1 - playerid;
	auto message = pduel->new_message(MSG_HINT);
	message->write<uint8_t>(hint_type);
	message->write<uint8_t>(playerid);
	message->write<uint64_t>(description);
	return 0;
}
LUA_STATIC_FUNCTION(HintSelection, std::variant<card*, group*> to_be_highlighted, std::optional<bool> selection) {
	auto message = pduel->new_message(selection.value_or(true) ? MSG_CARD_SELECTED : MSG_BECOME_TARGET);
	if(auto* ppcard = std::get_if<card*>(&to_be_highlighted); ppcard) {
		message->write<uint32_t>(1);
		message->write((*ppcard)->get_info_location());
	} else {
		auto* pgroup = *std::get_if<group*>(&to_be_highlighted);
		message->write<uint32_t>(pgroup->container.size());
		for(auto& _pcard : pgroup->container)
			message->write(_pcard->get_info_location());
	}
...
LUA_STATIC_FUNCTION(SelectEffectYesNo, playerid_t playerid, card* pcard, std::optional<uint64_t> description) {
	check_action_permission(L);
	pduel->game_field->emplace_process<Processors::SelectEffectYesNo>(playerid, description.value_or(95), pcard);
	return yieldk({
		lua_pushboolean(L, pduel->game_field->returns.at<int32_t>(0));
		return 1;
	});
}
LUA_STATIC_FUNCTION(SelectYesNo, playerid_t playerid, uint64_t description) {
	check_action_permission(L);
	pduel->game_field->emplace_process<Processors::SelectYesNo>(playerid, description);
	return yieldk({
		lua_pushboolean(L, pduel->game_field->returns.at<int32_t>(0));
		return 1;
	});
...
LUA_STATIC_FUNCTION(SelectDisableField, playerid_t playerid, uint8_t count, uint16_t location_self, uint16_t location_oppo,
					std::optional<uint32_t> extra_zone_filter, std::optional<bool> whole_field) {
	check_action_permission(L);
	bool all_field = whole_field.value_or(false);
	...
	filter |= extra_zone_filter.value_or(filter);
	uint32_t ct1 = 0, ct2 = 0, ct3 = 0, ct4 = 0, plist = 0, flag = 0xffffffff;
	if(location_self & LOCATION_MZONE) {
	...
	if(location_self & LOCATION_SZONE) {
	...
	if(location_oppo & LOCATION_MZONE) {
	...
	if(location_oppo & LOCATION_SZONE) {
...
LUA_STATIC_FUNCTION(SelectFieldZone, playerid_t playerid, uint8_t count, uint16_t location_self, uint16_t location_oppo,
					std::optional<uint32_t> extra_zone_filter) {
	check_action_permission(L);
	auto filter = extra_zone_filter.value_or(0xe0e0e0e0);
	filter |= pduel->game_field->is_flag(DUEL_EMZONE) ? 0x800080 : 0xE000E0;
	filter |= pduel->game_field->get_pzone_zones_flag();
...
LUA_STATIC_FUNCTION(GetCustomActivityCount, uint32_t counter_id, playerid_t playerid, ActivityType activity_type) {
...
LUA_STATIC_FUNCTION(GetCardFromCardID, uint32_t id) {
...
LUA_STATIC_FUNCTION(TagSwap, playerid_t playerid) {
	check_action_permission(L);
	pduel->game_field->tag_swap(playerid);
	return yield();
}
LUA_STATIC_FUNCTION(GetPlayersCount, playerid_t playerid) {
	lua_pushinteger(L, pduel->game_field->get_player_count(playerid));
	return 1;
}
LUA_STATIC_FUNCTION(SwapDeckAndGrave, playerid_t playerid) {
	check_action_permission(L);
