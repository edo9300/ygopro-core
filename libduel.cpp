...
LUA_STATIC_FUNCTION(ReturnToField, card* pcard, std::optional<uint8_t> position, std::optional<uint32_t> zone) {
	...
	pduel->game_field->move_to_field(pcard, pcard->previous.controler, pcard->previous.controler, pcard->previous.location,
									 position.value_or(pcard->previous.position), TRUE, 1, zone.value_or(0xff), FALSE, LOCATION_REASON::RETURN_TOFIELD);
	return yieldk({
		lua_pushboolean(L, pduel->game_field->returns.at<int32_t>(0));
		return 1;
	});
...
LUA_STATIC_FUNCTION(GetLocationCount, playerid_t playerid, uint8_t location, std::optional<uint8_t> uplayer, std::optional<LOCATION_REASON> reason, std::optional<uint32_t> zone) {
	uint32_t list = 0;
	lua_pushinteger(L, pduel->game_field->get_useable_count(nullptr, playerid, location,
															...
															reason.value_or(LOCATION_REASON::TOFIELD),
															zone.value_or(0xff), &list));
	lua_pushinteger(L, list != 0);
	return 2;
}
LUA_STATIC_FUNCTION(GetMZoneCount, playerid_t playerid, std::variant<Nil, card*, group*> card_or_group, std::optional<playerid_t> uplayer, std::optional<LOCATION_REASON> reason, std::optional<uint32_t> zone) {
	bool swapped = false;
...
LUA_STATIC_FUNCTION(GetUsableMZoneCount, playerid_t playerid, std::optional<uint8_t> uplayer) {
	...
	int32_t ct1 = pduel->game_field->get_tofield_count(nullptr, playerid, LOCATION_MZONE, actual_uplayer, LOCATION_REASON::TOFIELD, zone, &flag1);
	...
	int32_t limit = pduel->game_field->get_mzone_limit(playerid, actual_uplayer, LOCATION_REASON::TOFIELD);
	if(count > limit)
		count = limit;
