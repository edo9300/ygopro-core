...
LUA_STATIC_FUNCTION(EnableGlobalFlag, uint32_t flag_to_enable) {
	pduel->game_field->core.global_flag |= flag_to_enable;
	return 0;
}
...
LUA_STATIC_FUNCTION(RegisterFlagEffect, uint8_t playerid, uint32_t code, uint32_t reset, uint32_t flag, uint32_t count, uint32_t lab) {
	if(playerid != 0 && playerid != 1)
		return 0;
...
LUA_STATIC_FUNCTION(Destroy, uint32_t reason, std::optional<uint16_t> dest) {
	check_action_permission(L);
	auto actual_dest = dest.value_or(LOCATION_GRAVE);
	...
	if(auto [pcard, pgroup] = lua_get_card_or_group(L, 1); pcard)
		pduel->game_field->destroy(pcard, pduel->game_field->core.reason_effect, reason, reasonplayer, PLAYER_NONE, actual_dest, 0);
	else
		pduel->game_field->destroy(pgroup->container, pduel->game_field->core.reason_effect, reason, reasonplayer, PLAYER_NONE, actual_dest, 0);
	return yieldk({
...
LUA_STATIC_FUNCTION(ChainAttack, std::optional<card*> chain_attack_target) {
	...
	if(chain_attack_target.has_value())
		pduel->game_field->core.chain_attack_target = *chain_attack_target;
	return 0;
}
...
LUA_STATIC_FUNCTION(IsPlayerCanDraw, uint8_t playerid, uint32_t count) {
	if(playerid != 0 && playerid != 1) {
		lua_pushboolean(L, 0);
		return 1;
	}
